...
PS_OUT PS(VS_OUT_RIM input) : SV_Target
{
    ...
    if (texColor.r > 0.3f)
    {
       // 0 ~ 1로 반복되는 파형 생성 (sin 기반)
        float flash = sin(g_fGlobalTime * 3.14159f); // 주기 = 2초 (sin(pi * t))
        flash = abs(flash); // 음수 제거 (0 ~ 1)
        
        float hitAmount = smoothstep(0.8f, 1.f, flash); // 천천히 밝아지고 부드럽게 어두워짐
        float3 pureWhite = float3(1.0f, 0.1f, 0.1f);
        float alpha = lerp(0.5f, 1.0f, hitAmount);

        output.c3 = float4(pureWhite, alpha);
