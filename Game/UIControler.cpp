...
void InGameUIControler::init()
{
	...
	m_vEndingText = PToA->MakeUIs("../Resources/Prefab/UI_Ending_Text.uis.json");
	m_vEndingImage = PToA->MakeUIs("../Resources/Prefab/UI_Ending_Image.uis.json");
}
...
void InGameUIControler::UpdateEnding()
{
	...
	if (oneFrame)
	{
		...
		UI->AddUIList(m_vEndingImage);
		UI->AddUIList(m_vEndingText);

		m_pEndingFade->m_bRun = true;
		m_pEndingFade->m_bRender = true;

		for (auto& pUI : m_vEndingImage)
		{
			pUI->m_bRun = true;
			pUI->m_bRender = true;
		...
		for (auto& pUI : m_vEndingText)
		{
			pUI->m_bRun = true;
			pUI->m_bRender = true;
			m_vEndingText[0]->SetColor(Color(0.f, 0.f, 0.f, 1.f));
		}

		wstring ending = L"팀 명\n\n\n";
		ending += L"DD44";
		m_vEndingText[0]->SetText(ending);
		m_vEndingText[0]->SetPosition(Vec3(0.f, -(g_windowSize.y * 0.7f), 0.f));
		...
		ending += L"김예린\n\n";
		ending += L"이윤석";
		m_vEndingText[1]->SetText(ending);
		m_vEndingText[1]->SetPosition(Vec3(0.f, -(g_windowSize.y * 1.3f), 0.f));

		ending = L"특별 감사\n\n\n";
		ending += L"김명균 원장님";
		m_vEndingText[2]->SetText(ending);
		m_vEndingText[2]->SetPosition(Vec3(0.f, -(g_windowSize.y * 1.8f), 0.f));

		ending = L"플레이  해주셔서  감사합니다!";
		m_vEndingText[3]->SetText(ending);
		//m_vEndingText[3]->SetPosition(Vec3(0.f, -(g_windowSize.y * 3.f), 0.f));
		m_vEndingText[3]->SetPosition(Vec3(0.f, -(g_windowSize.y * 2.6f), 0.f));
	}
	...
	auto trigger = m_vEndingText[3]->GetPosition().y;

	static float fase[7] = {
	trigger + (g_windowSize.y * 0.2f), // 0
	trigger + (g_windowSize.y * 0.5f), // 1
	trigger + (g_windowSize.y * 0.8f), // 2
	trigger + (g_windowSize.y * 1.1f), // 3
	trigger + (g_windowSize.y * 1.4f), // 4
	trigger + (g_windowSize.y * 1.7f), // 5
	//-(g_windowSize.y * 0.2f)  // 6
	};

	if (trigger < 0.0f)
	{
		for(auto& pUI : m_vEndingText)
			pUI->AddPosition(Vec3(0.f, TIMER->GetDeltaTime() * 70.f, 0.f));
	}
	else
		bEnd = true;
	...
	for (int i = 0; i < 6; i++)
	{
		if (trigger >= fase[i] && trigger < fase[i] + (g_windowSize.y * 0.1f))
		{
			// 0 ~ 1 선형
			auto temp = (trigger - (fase[i] + (g_windowSize.y * 0.1f))) / (g_windowSize.y * 0.1f);
			m_vEndingImage[i]->SetColor(Color(0.1f, 0.1f, 0.1f, temp));
		}
		else if (trigger >= fase[i] + (g_windowSize.y * 0.1f) && trigger < fase[i] + (g_windowSize.y * 0.2f))
		{
			m_vEndingImage[i]->SetColor(Color(0.1f, 0.1f, 0.1f, 0.f));
		}
		else if (trigger >= fase[i] + (g_windowSize.y * 0.2f) && trigger < fase[i] + (g_windowSize.y * 0.3f))
		{
			auto temp = (trigger - (fase[i] + (g_windowSize.y * 0.2f))) / (g_windowSize.y * 0.1f);
			m_vEndingImage[i]->SetColor(Color(0.1f, 0.1f, 0.1f, -temp));
		}
		else if (trigger >= fase[i] + (g_windowSize.y * 0.3f))
			m_vEndingImage[i]->SetColor(Color(0.1f, 0.1f, 0.1f, -1.f));
	}
	...
	if (bEnd)
	{
		...
		m_vEndingImage[7]->SetColor(Color(0.f, 0.f, 0.f, tempTime));
		m_vEndingImage[8]->SetColor(Color(0.f, 0.f, 0.f, tempTime));
	}
